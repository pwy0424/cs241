...
{
	d->entry = malloc(sizeof(dictionary_entry_t));
	//d->entry->key = malloc(sizeof(char));
	d->entry->key = NULL;
	//d->entry->value = malloc(sizeof(char));
	d->entry->value = NULL;
	d->entry->next = NULL;
}
...
int dictionary_add(dictionary_t *d, const char *key, const char *value)
{
	...
	{
		if(strcmp(temp->next->key,key) == 0) return KEY_EXISTS;
		temp = temp->next;
	}

	dictionary_entry_t* new = malloc(sizeof(dictionary_entry_t));
...
int dictionary_parse(dictionary_t *d, char *key_value)
{
	...
	if(i == 0) return ILLEGAL_FORMAT;
	if(i == length) return ILLEGAL_FORMAT;
	if(key_value[i+1] != ' ') return ILLEGAL_FORMAT;
...
const char *dictionary_get(dictionary_t *d, const char *key)
{
	...
	{
		//printf("%s\n",temp->key);
		if(strcmp(temp->next->key, key) == 0) break;
		temp = temp->next;
	}

	if(temp->next != NULL) return temp->next->value;
	else return NULL;
}
...
int dictionary_remove(dictionary_t *d, const char *key)
{
	...
	/*if (temp->key == key)
	{
		dictionary_entry_t* remove = temp;
		d->entry = temp->next;
		free(remove->key);
		free(remove->value);
		free(remove);
	...
	if(temp->next != NULL) 
	{
		dictionary_entry_t* toremove = temp->next;
		temp->next = toremove->next;

		free(toremove);
		return 0;
	}
	else return NO_KEY_EXISTS;
}
...
{
	dictionary_entry_t* remove = d->entry;
	dictionary_entry_t* temp = d->entry->next;
	while(temp != NULL)
	{
/*		if(remove == d->entry)
		{
